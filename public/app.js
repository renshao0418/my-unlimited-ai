...
(() => {
  ...
  // ====== 用户输入导航状态 ======
  /** 当前高亮的用户消息索引，-1 表示未激活 / 在底部 */
  let currentUserMsgIndex = -1;
  /** 双击判定间隔（毫秒） */
  const DOUBLE_CLICK_MS = 300;
  let _btn1ClickTimer = null;
  let _btn2ClickTimer = null;

  function getAllUserRows() {
    return Array.from(chatEl.querySelectorAll(".row.user"));
  }
  ...
  // 侧边按钮1：单击向后（更早的）遍历用户输入；双击回到顶部
  scrollToUserBtn.addEventListener("click", () => {
    if (_btn1ClickTimer) {
      // 双击：取消单拟定时器，执行顶部跳转
      clearTimeout(_btn1ClickTimer);
      _btn1ClickTimer = null;
      historyWrap.scrollTo({ top: 0, behavior: "smooth" });
      currentUserMsgIndex = -1;
      return;
    }
    _btn1ClickTimer = setTimeout(() => {
      _btn1ClickTimer = null;
      // 单击：向后（更早的）遍历用户消息
      const userRows = getAllUserRows();
      if (userRows.length === 0) return;
      if (currentUserMsgIndex === -1) {
        // 首次使用：定位到最近一条用户输入
        currentUserMsgIndex = userRows.length - 1;
      } else if (currentUserMsgIndex > 0) {
        // 还有更早的，就往前一条
        currentUserMsgIndex--;
      }
      // 否则已经是最旧的一条，保持不动
      userRows[currentUserMsgIndex].scrollIntoView({ behavior: "smooth", block: "start" });
    }, DOUBLE_CLICK_MS);
  });

  // 侧边按钮2：单击向前（更新的）遍历用户输入；双击平滑回到底部
  scrollToBottomBtn.addEventListener("click", () => {
    if (_btn2ClickTimer) {
      clearTimeout(_btn2ClickTimer);
      _btn2ClickTimer = null;
      // 双击：平滑滚动到底部
      scrollToBottom(true);
      currentUserMsgIndex = -1;
      return;
    }
    _btn2ClickTimer = setTimeout(() => {
      _btn2ClickTimer = null;
      // 单击：向前（更新的）遍历用户消息
      const userRows = getAllUserRows();
      if (userRows.length === 0) return;
      if (currentUserMsgIndex === -1 || currentUserMsgIndex >= userRows.length - 1) {
        // 未激活 或 已到最新：跳到最新一条用户输入
        currentUserMsgIndex = userRows.length - 1;
      } else {
        // 还有更新的，就往后一条
        currentUserMsgIndex++;
